...
def add_product_to_cart():
    ...
    try:
        ...
        if quantity and pCode and request.method == 'POST':
            # conn = mysql.connect()
            cursor = mysql.connection.cursor()
            # cursor = conn.cursor(pymysql.cursors.DictCursor)
            cursor.execute("SELECT * FROM products WHERE pCode=%s", pCode)
            row = cursor.fetchone()

            itemArray = {row['pCode']: {'pName': row['pName'], 'pCode': row['pCode'], 'quantity': quantity,

                                        'price': row['price'], 'picture': row['picture'], 'total_price': quantity * row['price']}}
            ...
            if 'cart_item' in session:
                if row['pCode'] in session['cart_item']:
...
def products():
    try:
        ...
        return render_template('cart_item.html', products=rows)
    except Exception as e:
        print(e)
    finally:
        # conn.close()
